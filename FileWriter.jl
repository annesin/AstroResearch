...
function fileWriter(m, a, percent,t = "1000P",hParam = 0.01,fileSave = "AutoSave",MemorySave="hybrid")
    ...
    record, rowNumber, stability = Master("Test_$([m[1],m[2],m[3],a[1],a[3]]).txt",true,"AutoSave_$([m[1],m[2],m[3],a[1],a[3]])",0,MemorySave) #run simulation, get stability. Note that we're saving stuff for the heck of it
