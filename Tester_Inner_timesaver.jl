include("NBF_timesaver.jl")

function StabilityFinder2_preset(m, a1, a2, theta, percent, t="1000P", hParam=0.01, fileSave="AutoSaveIn")
    println("Stab_preset")
    ...
    precision = 100 #filler bc we want while loop to run
    ...
    println("before while, a1 is $a1, precision is $precision")
    while precision > percent || islandcheck == false #islandcheck only evaluated if precision <= percent
        println("in while, a1 is $a1, precision is $precision")
    #while a1 > .5
        ...
        y = open("Total_p$([m[1],m[2],m[3],a2,theta,percent]).txt","a") #creating file to record outputs, "a" means append
